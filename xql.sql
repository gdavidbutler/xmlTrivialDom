...
CREATE TRIGGER IF NOT EXISTS "XqlAv_ii" INSTEAD OF INSERT ON "XqlAv" BEGIN
 INSERT INTO "XqlC"("v")SELECT NEW."v" WHERE NOT EXISTS(SELECT * FROM "XqlC" WHERE "v"=NEW."v");
 INSERT INTO "XqlT"("v","o")SELECT NEW."n",INSTR(NEW."n",':')+1 WHERE NEW."n" IS NOT NULL AND NOT EXISTS(SELECT * FROM "XqlT" WHERE NEW."n" IS NOT NULL AND "v"=NEW."n");
 INSERT INTO "XqlA"("i","e","o","v","n")VALUES(NEW."i",NEW."e",COALESCE(NEW."o",(SELECT MAX(o)+1 FROM "XqlA" WHERE e=NEW.e),0),(SELECT "i" FROM "XqlC" WHERE "v"=NEW."v"),(SELECT "i" FROM "XqlT" WHERE NEW."n" IS NOT NULL AND "v"=NEW."n"));
END;
