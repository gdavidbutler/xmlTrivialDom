...
CREATE TRIGGER IF NOT EXISTS "XqlT_bd" BEFORE DELETE ON "XqlT" BEGIN
 ...
 ;
END;
CREATE TRIGGER IF NOT EXISTS "XqlT_au_v" AFTER INSERT ON "XqlT" WHEN NEW."o"!=INSTR(NEW."v",NEW."o")+1 BEGIN
 UPDATE "XqlT" SET "o"=INSTR(NEW."v",':')+1 WHERE "i"=NEW."i";
END;
...
END;
CREATE TRIGGER IF NOT EXISTS "XqlT_au_v" AFTER UPDATE OF "v" ON "XqlT" WHEN NEW."v"!=OLD."v" BEGIN
 UPDATE "XqlT" SET "o"=INSTR(NEW."v",':')+1 WHERE "i"=NEW."i";
END;
...
CREATE TRIGGER IF NOT EXISTS "XqlEv_t_iu" INSTEAD OF UPDATE OF "t" ON "XqlEv" BEGIN
 INSERT INTO "XqlT"("v","o")SELECT NEW."v",INSTR(NEW."v",':')+1 WHERE NEW."t"=0 AND NOT EXISTS(SELECT * FROM "XqlT" WHERE "v"=NEW."v");
 INSERT INTO "XqlC"("v")SELECT NEW."v" WHERE NEW."t"=1 AND NOT EXISTS(SELECT * FROM "XqlC" WHERE "v"=NEW."v");
 UPDATE "XqlE" SET("t","v")=(NEW."t",IFNULL((SELECT "i" FROM "XqlT" WHERE(0,"v")=(NEW."t",NEW."v")),(SELECT "i" FROM "XqlC" WHERE(1,"v")=(NEW."t",NEW."v"))))WHERE("i","t","v")!=(NEW."i",NEW."t",IFNULL((SELECT "i" FROM "XqlT" WHERE(0,"v")=(NEW."t",NEW."v")),(SELECT "i" FROM "XqlC" WHERE(1,"v")=(NEW."t",NEW."v"))));
END;
...
-- XQL attribute view
CREATE VIEW IF NOT EXISTS "XqlAv"("i","e","o","v","n")AS SELECT "t0"."i","t0"."e","t0"."o","t1"."v","t2"."v" FROM "XqlA" "t0" JOIN "XqlT" "t1" ON "t1"."i"="t0"."v" LEFT JOIN "XqlT" "t2" ON "t2"."i"="t0"."n";
...
CREATE TRIGGER IF NOT EXISTS "XqlAv_v_iu" INSTEAD OF UPDATE OF "v" ON "XqlAv" BEGIN
 INSERT INTO "XqlT"("v","o")SELECT NEW."v",INSTR(NEW."v",':')+1 WHERE NOT EXISTS(SELECT * FROM "XqlT" WHERE "v"=NEW."v");
 UPDATE "XqlA" SET "v"=(SELECT "i" FROM "XqlT" WHERE "v"=NEW."v")WHERE "i"=NEW."i" AND "v"!=(SELECT "i" FROM "XqlT" WHERE "v"=NEW."v");
END;
CREATE TRIGGER IF NOT EXISTS "XqlAv_n_iu" INSTEAD OF UPDATE OF "n" ON "XqlAv" BEGIN
 INSERT INTO "XqlT"("v","o")SELECT NEW."n",INSTR(NEW."n",':')+1 WHERE NEW."n" IS NOT NULL AND NOT EXISTS(SELECT * FROM "XqlT" WHERE NEW."n" IS NOT NULL AND "v"=NEW."n");
